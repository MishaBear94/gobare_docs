...
description: "You are past `curl` and putting this in a service"
---
...
changes — see the [changelog](/changelog) — so the package makes the semver
...
The [Python client](/python) is the same thing, from the same table, by a
...
infinite loop at full rate. See [limits.md](/limits).
...
the client keeps them apart. See [events.md](/events).
...
same as sending none. See [idempotency.md](/idempotency).

## Reading a single file, without the library

If you would rather see the whole loop than add a dependency,
[`run-session.ts`](/run-session) is the same job in one screen, importing
...
you see in [troubleshooting.md](/troubleshooting), and every refusal shape is
in [errors.md](/errors).

## Next

- [quickstart.md](/quickstart) — the same path in `curl`, with the token step
- [events.md](/events) — what the frames mean
- [required-actions.md](/required-actions) — letting the agent call your code
