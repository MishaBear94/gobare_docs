---
title: "Limits"
description: "You are planning load, or you got a 429"
---
...
**`GET /v1/events?root=` is one stream, against both ceilings, however many
sessions are in the tree it covers.** A crew of ten watched through its root's
`?root=` stream costs one slot, not ten — the same connection just carries more
sessions' frames.
...
with your cursor and nothing persisted is lost — see [events.md](/events).
...
**A crew of N costs N slots, against this same ceiling** — a coordinator and
its workers are ordinary sessions, counted the same as any other. `fork`
spends one too: it is a second running computer, not a cheaper copy. Twenty-five
was sized for one task at a time, not for a room of bots left running.

**A session tree is only walked 16 levels deep.** `parent_session_id` itself
is not depth-checked at creation — a seventeenth level is created without
complaint. It is `?root=`, on both `GET /v1/sessions` and the event stream,
that stops finding members past that depth, so a chain that deep silently
drops out of every list and stream built on `root=` while still existing and
still running. Sixteen holds an ordinary crew; it is not a ceiling you are
meant to approach. See [sessions.md](/sessions#crews).
...
survive it; see [the artifacts section of the quickstart](/quickstart).
...
- [idempotency](/idempotency) — retrying safely
- [troubleshooting](/troubleshooting) — you hit one and are not sure why
