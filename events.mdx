...
| `GET /v1/events` | Every session the organization owns — or `?root=` to scope it to one crew, see [sessions.md#crews](/sessions#crews) |
...
| `file.changed` | `{path, changeType, diff?, toolCallId?}` — `changeType` is created / modified / deleted, and `toolCallId` is which call caused it, when there was one |
...
| `approval.resolved` | yes | Permission was granted or refused. A refusal carries `name`, a `code` (`denied_read_only`, `denied_by_rule`, `approval_timeout`, `credential_inspection_denied`, `aborted`) and the reason the agent was given — so a tool the platform blocked is tellable from one the model never tried |
