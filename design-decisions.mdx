...
description: "You want to know why the API behaves as it does"
---
...
[`input.steer`](/input) with no turn running is a `conflict` rather than a new turn.
...
**This is not the same decision as multi-agent, and reading it as one is the
mistake to avoid:**

- **An agent spawning a sub-agent** — still not offered, deliberately, for the
  reason above. Nothing here has changed.
- **Your service orchestrating several peer sessions** — offered. See
  [sessions.md](/sessions#crews) and
  [Let several agents work on one thing](/guides/agents-that-work-together).

The two look similar from outside a single session and are unrelated inside
one: a sub-agent would be the runtime deciding to create work; a crew is your
service deciding to, through the same `POST /v1/sessions` you already call.
...
- **Errors carry a code, not a type.** Sixteen codes, listed in
  [errors.md](/errors). Three share `429` and recover differently.
...
- **A crew is not a new noun.** It is a tree on the `forked_from` column,
  which has held production rows since migration 11. The API field is
  `parent_session_id` because a coordinator starting a worker is not a fork.
  There is no crew object, and nothing to create or delete.

## Next

- [troubleshooting](/troubleshooting) — something behaves unexpectedly
- [changelog](/changelog) — what changed recently
