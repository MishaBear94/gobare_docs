---
title: "Stream the agent's progress into your UI"
description: "Show my users what the agent is doing, live"
---
...
A token and a model, as in the [quickstart](/quickstart). The stream is
...
[limits.md](/limits). In a multi-tenant interface, hold one stream per
...
| `429` opening a stream | The per-organization stream ceiling | Hold one stream per user, not one per component — [limits.md](/limits) |
