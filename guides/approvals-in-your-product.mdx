---
title: "Require human approval before the agent acts"
description: "Make a person approve something before the agent does it"
---
...
```mermaid
flowchart TD
  act["agent decides to act"] --> park["the turn parks"]
  park --> show["your app shows it to a person"]
  show --> decide["they decide"]
  decide --> answer["you send the answer"]
  answer --> cont["the agent continues"]
```

## Before you start

A token and a model, as in the [quickstart](/quickstart). **You do not
...
while you were deciding. All three are normal; see [input.md](/input).
...
[webhooks.md](/webhooks) has the formula.
...
- [required-actions.md](/required-actions) — the full object, deadlines, expiry
- [input.md](/input) — the other three things you can send a session
- [Stream the agent's progress into your UI](/guides/showing-the-agents-work) — showing the investigation, not just the question
