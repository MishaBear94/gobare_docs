---
title: "Put an agent behind your own API"
description: "Hand work to an agent from my own API and collect it later"
---
...
```mermaid
flowchart TD
  subgraph fast["your request handler — milliseconds"]
    direction LR
    req["POST /reports"] --> create["create session"] --> ack["202 + job id"]
  end
  subgraph slow["your worker — minutes later"]
    direction LR
    work["agent works,<br/>calls your functions"] --> collect["collect the result"] --> tell["tell whoever asked"]
  end
  ack --> work
```
...
A token and a model, as in the [quickstart](/quickstart). Nothing here
...
[Require human approval](/guides/approvals-in-your-product#the-type-you-did-not-expect).
...
- [Require human approval before the agent acts](/guides/approvals-in-your-product) — when a person must decide
- [Stream the agent's progress into your UI](/guides/showing-the-agents-work) — showing the work, not a spinner
