...
for await (const event of frames(stream)) {
  // "session.action_required" is the *webhook* topic for this — see
  // required-actions.md. On this SSE stream the same fact arrives as
  // `tool.required`, and filtering on the other name means this switchboard
  // receives nothing at all.
  if (event.type !== "tool.required") continue;
...
await post(`/v1/sessions/${to}/files`, { files: [{ type: "inline", path: "/workspace/in/report.md", data: body }] });
```
