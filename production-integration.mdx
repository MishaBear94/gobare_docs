...
description: "It works, and now it has to keep working"
---
The [quickstart](/quickstart) gets one agent working. This page is the
...
It is not a package, on purpose — see [design-decisions.md](/design-decisions). It is
...
the middle. See [events.md](/events).

## Several sessions, one piece of work

The loop above is for one session. When the work is split across several —
one agent researches, another writes, a third reviews — join them into a tree
instead of tracking your own list of which sessions belong together:

<CodeGroup>

```bash curl
COORDINATOR=$(curl -s -X POST $GOBARE_API/v1/sessions \
  -H "Authorization: Bearer $GOBARE_TOKEN" -H 'content-type: application/json' \
  -d '{"title":"researcher"}' | jq -r .id)

curl -s -X POST $GOBARE_API/v1/sessions \
  -H "Authorization: Bearer $GOBARE_TOKEN" -H 'content-type: application/json' \
  -d "{\"title\":\"coder\",\"parent_session_id\":\"$COORDINATOR\"}"

# Both, in one call: itself included, not just direct children.
curl -s "$GOBARE_API/v1/sessions?root=$COORDINATOR" -H "Authorization: Bearer $GOBARE_TOKEN"
```

```typescript TypeScript
const coordinator = await call("POST", "/sessions", { title: "researcher" });
const worker = await call("POST", "/sessions", { title: "coder", parent_session_id: coordinator.id });

// Both, in one call: itself included, not just direct children.
const crew = await call("GET", `/sessions?root=${coordinator.id}`);
```

```python Python
coordinator = call("POST", "/sessions", {"title": "researcher"})
worker = call("POST", "/sessions", {"title": "coder", "parent_session_id": coordinator["id"]})

# Both, in one call: itself included, not just direct children.
crew = call("GET", f"/sessions?root={coordinator['id']}")
```

</CodeGroup>

The same `?root=` reaches the event stream and the usage endpoint. Left off
either one, you get the whole organization's sessions or the whole
organization's usage — not an error, just not what you meant. See
[sessions.md](/sessions#crews) and
[limits.md](/limits) for what a crew costs: each session is still its own
sandbox, billed and slotted like any other.

How agents in a crew actually hand work to each other — a function tool your
service answers, not a call between sandboxes — is its own page:
...
- [input.md](/input) — the four things you can send a session, and what each refuses
- [events.md](/events) — stream instead of polling, and resume where you dropped
- [webhooks.md](/webhooks) — be told rather than watch, and verify the delivery
- [required-actions.md](/required-actions) — let the agent call your code
- [idempotency.md](/idempotency) — a retry that does not act twice
- [pagination.md](/pagination) — collections longer than one page
- [limits.md](/limits) — every ceiling, with its number
- [troubleshooting.md](/troubleshooting) — a symptom you can look up, and [errors.md](/errors) for the shape every refusal arrives in

And the [guides](/guides) put them together: six complete programs,
