---
title: "Agents"
description: "You want every session in one part of your product to start the same way"
---
...
**What this saves.** `model`, `model_credential_id`, `instructions`, `tools`,
`text`, `approval_mode` and `permission_rules`.

The last two are the safety settings, and they used not to travel: an agent
had no field for either, so a support bot saved as the `read_only`
investigator it was built from started sessions that were `auto` — the mode
that acts without asking — with nothing anywhere saying the setting had not
come along. It travels now. An agent that names neither still starts sessions
at `auto`; the difference is that you can say so once instead of on every
`POST /v1/sessions`. See
[Approval mode and permission rules](/sessions#approval-mode-and-permission-rules)
...
| `approval_mode` | **Per field.** Sending a mode inline keeps the agent's `permission_rules` |
| `tools`, `text`, `permission_rules` | **Replaced whole, never merged.** A half-merged list is ambiguous about which half won, and for rules the half that survives silently is a permission |
...
shape they arrive in is [errors.md](/errors).

## Next

- [sessions.md](/sessions) — every field an agent can hold, described in full
- [tools.md](/tools) — the `tools` an agent carries
- [reference](/api-reference/check-the-access-token) — the `/v1/agents` endpoints
